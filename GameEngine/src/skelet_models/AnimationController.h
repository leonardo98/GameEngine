...
public:
	...
	bool Import3DFromFile( const std::string& pFile = "")
	{
		...
		if (!oxygine::file::exists(m_ModelPath.c_str()))
		{
		...
		if (_scenes[_curScene]) { 
			...
			oxygine::log::messageln("Import of _curScene %s succeeded.", m_ModelPath.c_str());
		}
		 else {
			 oxygine::log::messageln( _importers[_curScene]->GetErrorString());
		 }
	...
	{		
		...
		oxygine::log::messageln("drawing objects");
	...
	{
		// Check if file exists
		if (!oxygine::file::exists(fileName.c_str()))
		{
