...
{
	OX_ASSERT(pNodeAnim->mNumRotationKeys > 0);
...
void CalcInterpolatedRotation(aiQuaternion& Out, float AnimationTime, const aiNodeAnim* pNodeAnim)
{
	...
	OX_ASSERT(NextRotationIndex < pNodeAnim->mNumRotationKeys);
	float DeltaTime = pNodeAnim->mRotationKeys[NextRotationIndex].mTime - pNodeAnim->mRotationKeys[RotationIndex].mTime;
	float Factor = (AnimationTime - (float)pNodeAnim->mRotationKeys[RotationIndex].mTime) / DeltaTime;
	OX_ASSERT(Factor >= 0.0f && Factor <= 1.0f);
...
{
	OX_ASSERT(pNodeAnim->mNumScalingKeys > 0);
...
void CalcInterpolatedScaling(aiVector3D& Out, float AnimationTime, const aiNodeAnim* pNodeAnim)
{
	...
	OX_ASSERT(NextScalingIndex < pNodeAnim->mNumScalingKeys);
...
{
	OX_ASSERT(pNodeAnim->mNumPositionKeys > 0);
...
void CalcInterpolatedPosition(aiVector3D& Out, float AnimationTime, const aiNodeAnim* pNodeAnim)
{
	...
	OX_ASSERT(NextPositionIndex < pNodeAnim->mNumPositionKeys);
