...
public:
	...
	inline void Mul(oxygine::Vector2 origin, float &x, float &y) const {
		x = _matrix[0][0] * origin.x + _matrix[1][0] * origin.y + _matrix[2][0];
		y = _matrix[0][1] * origin.x + _matrix[1][1] * origin.y + _matrix[2][1];
	}
	void Mul(const Matrix3x3 &transform) {
		Mul(transform._matrix);
	}
	inline void Mul(oxygine::Vector2 &p) const {
		float x = p.x;
	...
	void MakeRevers(const Matrix3x3 &transform) {
